...
Werden nacheinander *unabhängige Bernoulli-Experimente* durchgeführt, d.h. es gibt die Versuchsausgänge $1$ (Erfolg) und $0$ (Missverfolg), dann gibt die geometrische Verteilung entweder:

- **Variante A**: die Wahrscheinlichkeitsverteilung der Anzahl $X$ der Bernoulli-Versuche, die notwendig sind, um einen Erfolg zu haben (definiert auf der Menge  $\mathbb {N}$), oder
- **Variante B**: die Wahrscheinlichkeitsverteilung der Anzahl $Y$ der Fehlversuche vor dem ersten Erfolg. Diese Verteilung ist auf der Menge $\mathbb {N} _{0}$ definiert.

Die beiden Varianten stehen in der Beziehung $X=Y+1$.

| Geometrische Verteilung |   |
|:---------------------|:--------------------------------------------------------|
|**Definition**| Eine diskrete Zufallsgröße $X$ oder $Y$ mit den Parametern $p$ und $q=1-p$ (Wahrscheinlichkeiten für Erfolg / Misserfolg) genügt der geometrischen Verteilung $G(p)$, wenn für die Wahrscheinlichkeit, dass man genau $n$ Versuche benötigt, um zum ersten Erfolg zu kommen, gilt $${\displaystyle \operatorname {P} (X=n)=p(1-p)^{n-1}=pq^{n-1}\quad (n=1,2,\dotsc )},$$ bzw. für die Wahrscheinlichkeit, $n$ Fehlversuche vor dem ersten Erfolg zu haben, gilt $$\operatorname {P}(Y=n)=p(1-p)^{{n}}=pq^{{n}}\quad (n=0,1,2,\dotsc ).$$|
...
Entsprechend ergibt sich für die Wartenzeiten $0$ bis $10$ Mal Würfeln dann folgendes Bild:
...
### Verteilungsfunktion (CDF) in R

Um nun die Wahrscheinlichkeit zu finden, dass man höchstens drei Mal warten muss -- also maximal vier Mal würfelt bevor man die $6$ erhält -- braucht man die CDF (`pgeom`).
...
Entsprechend ergibt sich für die Fälle, dass die Wartezeit kleiner bzw. gleich $0, 1, ...$ ist dann folgendes Bild:
